...
export async function processGraphSync(
  ...
  try {
    ...
    // Step 5.6: Generate and commit CLAUDE.md + .gitattributes
    ...
    try {
      ...
      if (claudeMd) {
        // Build .gitattributes content (preserve existing + ensure CLAUDE.md entry)
        let gitattributes = '';
        try {
          const blob = await gitBackend.getBlob(ownerSlug, repo.slug, repo.defaultBranch || 'main', '.gitattributes');
          if (!blob.isBinary) gitattributes = blob.content;
        } catch { /* file doesn't exist yet */ }

        if (!gitattributes.includes('CLAUDE.md linguist-generated')) {
          const nl = gitattributes.length > 0 && !gitattributes.endsWith('\n') ? '\n' : '';
          gitattributes += `${nl}CLAUDE.md linguist-generated=true\n`;
        }

        const result = await gitBackend.commitFilesToRef(
          ownerSlug,
          repo.slug,
          repo.defaultBranch || 'main',
          [
